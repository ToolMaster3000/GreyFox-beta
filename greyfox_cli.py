...
def run_turn(messages, spinner_label="thinking", quiet=False):
    ...
    When stopped_reason is "answered", the text has already been streamed
    to stdout token-by-token as the model generated it -- callers should
    NOT print it again. For "needs_user" / "round_cap", the returned text
    is synthesized locally (never streamed), so callers still need to
    print it themselves."""
    for round_i in range(MAX_TOOL_ROUNDS_PER_TURN):
        ...
        def _on_delta(piece):
            ...
            if not quiet:
                if not streamed_any:
                    print()  # separate the streamed answer from the spinner line above
...
def print_banner():
    ...
    mascot_lines = [
        f"\033[1;38;2;{MASCOT_COLOR[0]};{MASCOT_COLOR[1]};{MASCOT_COLOR[2]}m"
        f"{_pad_center(line, content_width)}{RESET}"
        for line in MASCOT
    ]
