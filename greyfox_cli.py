...
try:
    if os.environ.get("GREYFOX_NO_READLINE"):
        readline = None  # escape hatch: GREYFOX_NO_READLINE=1 disables it entirely,
                          # in case a specific platform's readline build misbehaves
                          # with input() in ways this environment can't reproduce
    else:
...
def _atomic_write_text(path, text):
    """Write via a temp file + rename instead of a direct write_text(). Two
    things this guards against, both realistic here: a process getting
    killed mid-write (background kill/OOM/battery optimization -- a phone,
    not a server), and a concurrent reader (facts/plan/invariants get read
    on every single round via effective_messages()) observing a half-written
    file instead of either the old or new content cleanly."""
    path = Path(path)
...
_FACTS_LOCK = threading.Lock()  # guards the read-modify-write in tool_record_fact;
                                 # concurrent tool dispatch can run two record_fact
                                 # calls in the same round on separate threads, and
                                 # without this, whichever finishes last silently
                                 # discards the other's fact (classic lost update)
...
def _save_facts(facts):
    _atomic_write_text(FACTS_PATH, json.dumps(facts, indent=2))
...
def tool_record_fact(args):
    ...
    with _FACTS_LOCK:
        ...
        _save_facts(facts)
        total = len(facts)
    return f"recorded fact '{key}' ({total} facts total)"
...
def tool_update_plan(args):
    ...
    _atomic_write_text(PLAN_PATH, _render_plan(steps))
...
def check_server_alive(timeout=5):
    """Fast pre-flight check that llama-server is actually responding right
    now. Android can suspend/freeze a background process like llama-server
    while the app is idle -- exactly the battery-optimization risk setup.sh
    already warns about. Without this, a frozen server still accepts the
    next TCP connection (the OS does that regardless of whether the process
    is scheduled to run), so the real request just sits waiting up to its
    full 600s timeout with zero feedback -- indistinguishable, to a user
    watching a terminal, from an infinite hang. Called from call_model() and
    call_model_streaming() themselves (not from individual call sites like
    run_turn or capture_auto_plan) so every current and future caller is
    covered automatically, rather than relying on each one to remember it."""
    try:
...
def _raise_if_server_unreachable():
    if not check_server_alive():
        raise RuntimeError(
            f"llama-server at {LLAMA_SERVER_URL} isn't responding to a quick health "
            "check. It may have been frozen or killed by Android's battery "
            "optimization while idle -- see the battery-whitelist instructions from "
            "setup, or try 'bash setup.sh --run-only' to restart it."
        )
...
class Spinner:
    ...
    def stop(self, final=None):
        self._stop.set()
        if self._thread:
            # NOT a short timeout: a background thread still alive and
            # still writing raw \r + ANSI codes to the terminal at the exact
            # moment input()/readline sets up the next prompt is exactly how
            # you get "keystrokes vanish, prompt looks dead" -- and it's
            # intermittent because it depends on real-time thread scheduling,
            # which is far less predictable on a loaded/throttled phone than
            # on a fast dev machine. The spin loop's own sleep interval is
            # 0.08s, so an actually-healthy thread returns almost instantly
            # regardless; this timeout exists only so a truly stuck thread
            # (e.g. print() blocked on a flow-controlled terminal) can't
            # deadlock the whole program, not to let a slow-but-alive one
            # race the next prompt.
...
def save_session(name, messages):
    ...
    _atomic_write_text(path, json.dumps(messages, indent=2))
    return path
...
def capture_auto_plan(goal):
    ...
    _atomic_write_text(PLAN_PATH, "")
    _atomic_write_text(AUTO_GOAL_PATH, "")
...
def main():
    ...
    while True:
        try:
            ...
            # input()/readline itself failing is not something the rest of
            # the loop is built to handle -- surface it instead of dying
            # silently, and try to recover rather than crash the session
            print_callout("error", f"input error: {e}")
            if readline is not None:
                print_callout(
                    "warning",
                    "if this keeps happening, try running with GREYFOX_NO_READLINE=1 "
                    "to rule out a readline/terminal interaction issue.",
                )
            continue
