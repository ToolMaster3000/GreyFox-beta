...
def call_model_streaming(messages, tools=True, temperature=0.3, print_tokens=True, on_first_chunk=None):
...
def effective_messages(messages):
    """The list actually sent to the model: stored history plus a freshly
    re-read invariants block inserted right after the system prompt. Doesn't
    mutate or grow `messages` itself."""
    inv = invariants_message()
    if not inv:
        return messages
    return [messages[0], inv] + messages[1:]
...
def summarize_chunk(chunk):
    ...
    try:
        ...
        return f"(compaction summary failed: {e})"
...
def maybe_compact(messages):
    ...
    old = messages[1:-COMPACT_KEEP_RECENT]
    if not old:
        return False
...
def _call_non_streaming_as_choice(send_messages, quiet):
    ...
    token-by-token, but still functional."""
    t0 = time.time()
            ...
            if STREAMING_SUPPORTED:
                try:
                    ...
                    if spinner and not first_chunk_seen.is_set():
                        ...
                        spinner = None
                    if not quiet:
                        print(f"  {DIM}[warning] {e}{RESET}", file=sys.stderr)
...
def run_auto(messages, goal):
    ...
    for turn_i in range(MAX_AUTO_TURNS):
        ...
        text, reason, _stats = run_turn(messages, spinner_label=f"auto turn {turn_i + 1}")
        if reason != "answered":
...
def print_status_panel():
    ...
    for r in rows:
        print(f"{DIM}│{RESET} {r.ljust(width - 1)}{DIM}│{RESET}")
    print(f"{DIM}└{'─' * width}┘{RESET}")
    print()
...
def print_banner():
    ...
    print(f"{DIM}tools: search, http, files, python, notes, facts, termux -- used automatically when needed{RESET}")
    print()
    print_status_panel()
