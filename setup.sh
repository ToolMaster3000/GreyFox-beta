...
LLAMACPP_PIN="b3600"   # pin to a known-working tag/commit; bump deliberately
...
setup_venv() {
    ...
    # greyfox_cli.py only uses the stdlib, so nothing else is required here.
}
...
selftest() {
    ...
    start_server
    local resp
    resp=$(curl -s "http://127.0.0.1:$SERVER_PORT/v1/chat/completions" \
...
for arg in "$@"; do
    case "$arg" in
        ...
        --version) echo "GreyFox-CLI 0.1.0"; exit 0 ;;
