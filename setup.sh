...
install_cli() {
    ...
    # defensive: strip any stray trailing whitespace/CR that could corrupt
    # the path comparison below without being visibly obvious (e.g. if $0
    # or the environment got mangled by a CRLF-touched wrapper)
    script_dir="$(printf '%s' "$script_dir" | tr -d '\r' | sed 's/[[:space:]]*$//')"
