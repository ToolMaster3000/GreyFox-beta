...
| Runtime  | Termux — see [Installing Termux](#installing-termux) below (v0.118.0+)                             |
...
Termux application can be obtained on GitHub either from GitHub Releases for
version >= 0.118.0:
...
whitelist instructions (see [Battery optimization](#battery-optimization)
below), install a Termux autostart hook, and start the server + CLI.
Re-running `bash setup.sh` is idempotent.
...
Android's battery optimization can kill the background `llama-server` /
autostart hook. `setup.sh` prints a device-specific version of this at the
end of setup, based on your device manufacturer:

- **Xiaomi/Redmi (MIUI/HyperOS):** Security app -> Battery -> Termux -> No
  restrictions; also disable "MIUI Optimization" in Developer Options.
- **Samsung (One UI):** Device Care -> Battery -> Termux -> add to "Never
  sleeping apps".
- **Huawei (EMUI/HarmonyOS):** Battery settings -> Protected Apps -> enable
  for Termux.
