...
- **`/auto` run summary**: every exit path (`TASK_COMPLETE`, needs input, or
  the turn cap) ends with a box showing turns used, wall time, tokens
  generated, and a tool-usage tally. If Termux:API is installed, a
  notification fires too — useful since a run can take minutes on a slow
  phone CPU and you'll likely have tabbed away.
- **Colored, styled errors**: failures (a bad `/save`, a corrupted session,
  ...
  a bare `[error] ...` line.
- **Command history**: arrow-key history and line editing work in the input
  prompt (via `readline`), persisted across sessions in
  `~/greyfox-cli/history`. Unknown commands get a typo suggestion (`/atuo` →
  "did you mean /auto?") instead of just failing.
- **`/copy`**: copies the last reply to the clipboard via Termux:API.
